const ErrorHandler = (err, req, res, next) => {
	...
	if (err.name === 'InvalidTokenError') {
		return res.status(400).json({ success: false, erroerMessage: '사용할 수 없는 토큰입니다. 로그인을 해주세요.' });
	}
	if (err.name === 'AlreadyLoginError') {
		return res.status(400).json({ success: false, erroerMessage: '이미 로그인한 회원입니다.' });
	}
	if (err.name === 'ProductDosntExistError') {
		return res.status(400).json({ success: false, erroerMessage: '존재하지 않는 상품입니다.' });
	}
	if (err.name === 'NoPermissionError') {
		return res.status(400).json({ success: false, erroerMessage: '작성자가 아님으로 권한이 없습니다.' });
	}
